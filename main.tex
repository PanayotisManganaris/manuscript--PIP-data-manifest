...
\footnotetext{%
  \textsuperscript{a}School of Materials Engineering, Purdue University, West Lafayette, IN 47907, USA; E-mail: amannodi@purdue.edu
}
\footnotetext{%
  \textsuperscript{\dag}Electronic Supplementary Information (ESI) available:
  \url{https://github.com/PanayotisManganaris/manuscript--PIP-data-manifest}
...
used are discussed in \hyperref[sec:org07ee6cc]{Methodology}. The relatively large size of this
...
\captionof{figure}{\label{fig:lc_vs_pbe} Comparison of PBE computed pseudo-cubic lattice constants with crystallographic measures for lattice constants}
\end{center}

\begin{center}
\includegraphics[width=.9\linewidth]{./expval/hse_v_exp_LC.png}
\captionof{figure}{\label{fig:lc_vs_hse} Comparison of HSE computed pseudo-cubic lattice constants with crystallographic measures for lattice constants}
\end{center}
...
Fig \ref{fig:pairplots}(b) is showing the \acrshort{pbe} band gap compared to the
...
can statistically represent a sufficient Perovskite space. Fig \ref{fig:pairplots}(c)
...
Fig \ref{fig:pairplots}(d) presents the \gls{slme} values related to the
\acrshort{pbe} band gap. \gls{slme} is a proven proxy
for photovoltaic performance
\cite{yu-2012-ident-poten}. \gls{slme} measures the
absorption efficiency of light for the Perovskite. As Fig \ref{fig:pairplots}(d)
...
\caption{\label{fig:pca} PCA analysis on 495 PBE perovskite data}
\end{figure*}
...
\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./multivar/tsne_comp_bound_DecoE_clusters.png}
\caption{\label{fig:tsne} t-SNE analysis on fraction and elemental property descriptors}
\end{figure*}
...
\caption{\label{fig:cuts} Screening results of Decomposition Energy for (a) octahedral factor (b) Tolerance Factor (c) Bartel Tolerance Factor (d) Deviation from Cubicity.}
\end{figure*}
...
define deviation from cubicity. For each perovskite sample, we measure
the difference between b, c lattice parameter against a lattice,
showing in Equation \ref{eq:cubicity_l}. If the lattice deviation from cubicity is larger
...
angles $\alpha$, $\beta$, and $\gamma$, to make sure the angle also remain 90 degrees. We
calculated the difference of each angle versus 90 degrees, showing in
Equation \ref{eq:cubicity_aplha}, and take this as angular deviation from cubicity. We also
consider the samples that have more than 5\% of angular deviation from
cubicity are not cubic perovskites and exclude them. 
Fig \ref{fig:cuts}(d) shows
the screening results for average deviation from cubicity. Figure \ref{fig:cub_b}, \ref{fig:cub_c},  \ref{fig:cub_alpha}, \ref{fig:cub_beta}, and \ref{fig:cub_gamma}shows
the screening results for b, c lattice and $\alpha$,$\beta$,$\gamma$ angle respectively. Most of the
...
\begin{equation}\label{eq:cubicity_l}
DC_b=\frac{|b-a|}{a}
\end{equation}
\begin{equation}\label{eq:cubicity_aplha}
DC_\alpha=\frac{|\alpha-90^{\circ}|}{90^{\circ}}
\end{equation}
\begin{equation}\label{eq:cubicity_avg}
DC_{avg}=\frac{DC_b+DC_c+DC_\alpha+DC_\beta+DC_\gamma}{5}
\end{equation}


\subsubsection*{Octahedral factor, tolerance factor, and Bartel tolerance factor}
\label{sec:orgb08e02b}
...
factor.\cite{bartel-2019-new-toler} The formula of these three factors are shown in equation
\ref{eq:factor_o}, \ref{eq:factor_t} and \ref{eq:factor_b}. In our screening process, we set the criteria for Octahedral
...
be less than 4.18.  Fig \ref{fig:cuts}(a) shows the Octahedral factor versus
decomposition energy plot.  Fig \ref{fig:cuts}(b) shows the tolerance factor
...
decreases. Fig \ref{fig:cuts}(c) shows the Bartel tolerance factor versus
decomposition energy plot. Within the criteria of Bartel tolerance
factor, the decomposition energy is rapidly decreased.
\\
\\
octahedral factor:
\begin{equation}\label{eq:factor_o}
o=\frac{r_B}{r_X}
\end{equation}
\\
tolerance factor: 
\begin{equation}\label{eq:factor_t}
t=\frac{r_A+r_X}{\sqrt{2}(r_B+r_X)}
\end{equation}
\\
Bartel\cite{bartel-2019-new-toler} tolerance factor: 
\begin{equation}\label{eq:factor_b}
t_{Bartel}=\frac{r_X}{r_B}-[1-\frac{\frac{r_A}{r_B}}{ln(\frac{r_A}{r_B})}]
\end{equation}

\subsubsection*{Screening Results}
\label{sec:orgda8a4ea}
The Fig\ref{fig:screen_table} shows all 41 promising candidates we screened from our
dataset by using the cutoff criteria mentioned above. Fig\ref{fig:leftover}(a) shows the
...
Perovskite structure. In Fig\ref{fig:leftover}(b), the mixing distribution indicates
...
to a promising results but more risky.  The Figure\ref{fig:leftover_comp}, \ref{fig:screened_frac_A}, \ref{fig:screened_frac_B} and \ref{fig:screened_frac_X} shows
...
section \hyperref[sec:org923b673]{Comparing synthetic to physical data}

Also, we will explore methods for combining the insights of the \acrshort{pbe}
and \acrshort{hse} datasets in training surrogates, e.g. in section \hyperref[sec:org21fb725]{SLME calculations} the two \gls{slme} spectra could be used together to converge
...
\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./figs/cub_b-decomp.png}
\caption{\label{fig:cub_b} PBE Deviation from Cubicity plot at b axis}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./figs/cub_c-decomp.png}
\caption{\label{fig:cub_c} PBE Deviation from Cubicity plot at c axis}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./figs/cub_alpha_decomp.png}
\caption{\label{fig:cub_alpha} PBE Deviation from Cubicity plot at alpha}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./figs/cub_beta-decomp.png}
\caption{\label{fig:cub_beta} PBE Deviation from Cubicity plot at beta}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./figs/cub_gamma-decomp.png}
\caption{\label{fig:cub_gamma} PBE Deviation from Cubicity plot at gamma}
\end{figure*}
...
\caption{\label{fig:screened_frac_A} Element Fraction for A site elements among all screened perovskites}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./expval/Element_BFrac.jpg}
\caption{\label{fig:screened_frac_B} Element Fraction for B site elements among all screened perovskites}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.9\linewidth]{./expval/Element_XFrac.jpg}
\caption{\label{fig:screened_frac_X} Element Fraction for X site elements among all screened perovskites}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=.7\linewidth]{./figs/Screening_formula.png}
\caption{\label{fig:screen_table} Promising Perovskite Compounds Selected from Screening}
\end{figure*}
\end{document}