...
\end{itemize}
The perovskite structure has been widely investigated by material scientist and applied to industry products. A perovskite unit cell can be represented by a three-component ABX3 formula unit wherein A and B are cations with different oxidation states while X is an anion. BX6 octahedral network is the symbolic 3D perovskite structure, which are held in place by large A-site cations. This unique structure makes it incredibly tunable on electronic properties via engineering. In the past decades, numerous efforts and researches are devoted to halide perovskite as promising materials with excellent solar cell absorption[XXX]. ABX3 halide perovskites (HaPs) are consist of halogens such as I and Br as X-site anions, divalent elements such as Pb and Sn as B-site cations, and large mono-valent cations in A-site. Specially, the A-site cation can either be inorganic (e.g Cs, K, Rb) or organic ligands (e.g Methylammonium aka MA, Formamidinium aka FA). As the most common hybrid halide perovskite, MAPbI3 and FAPbI3 are proved to have extraordinary power conversion efficiencies between 20\% and 25\% when used as absorbers in single or multi junction solar cells.[XXX]
The most attractive features of halide perovskites are the unique tunability. Fundamentally, a perovskite structure is considered stable if A-site, B-site, and X-site ionic radii fulfill the well-known tolerance (t) and octahedral (µ) factors.[XXX] Under these restrictions, the halide perovskites still leave infinity possibility of element combination. Generally speaking, the design of halide perovskite can be divided into several aspects.
(1)	Compounds: 
(2)	Structure and phases:
(3)	Defects
(4)	Polymorphism

As the huge number of halide perovskites compounds, it rises challenges for experimentalists to screen over all possibilities. However, first principles simulation, such as density functional theory (DFT), have been systematically performed to model each of the factors mentioned above. More importantly, the expense of DFT modeling is much more acceptable when searching for new promising candidates in such boundaryless space. Recently, DFT simulations are known to be reliably applied for modeling structural information, heat of formation or decomposition, band gaps, optical absorption spectra, and defect formation energies of a variety of HaPs, with mixed accuracy compared to experiments.[XXX] 
